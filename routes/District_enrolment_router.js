...
router.get('/:district', function(req, res, next) {
    ...
    const dQuery =`select distinct(district) as district, sum(p1__enrolment__boys) as p1_boys, sum(p1__enrolment__girls) as p1_girls, 
    sum(p2__enrolment__boys) as p2_boys, sum(p2__enrolment__girls) as p2_girls, sum(p3__enrolment__boys) as p3_boys,
    sum(p3__enrolment__girls) as p3_girls, sum(p4__enrolment__boys) as p4_boys, sum(p4__enrolment__girls) as p4_girls,
    sum(p5__enrolment__boys)as p5_boys, sum(p5__enrolment__girls) as p5_girls, sum(p6__enrolment__boys) as p6_boys, 
    sum(p6__enrolment__girls) as p6_girls, sum(p7__enrolment__boys) as p7_boys, sum(p7__enrolment__girls) as p7_girls 
    from tbl_random  where district = '${nameOfDistrict}' group by district limit ${limit}`;
