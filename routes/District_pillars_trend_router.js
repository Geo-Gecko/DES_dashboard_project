...
router.get('/:district', function(req, res, next) {
    ...
    const limit = 1;
    const P1Query = `select distinct(district) as district, inspection.date_of_inspection as inspection_date, cast(avg(inspection.condition_of_school_building_and_compound) as unsigned) +
    ...
    const P2Query = `select distinct(district) as district, inspection.date_of_inspection as inspection_date, cast(avg(inspection.teacher_and_pupil_attendance) as unsigned) +
    ...
    const P3Query = `select distinct(district) as district, inspection.date_of_inspection as inspection_date, cast(avg(inspection.lesson_planning) as unsigned) +
    ...
    const P4Query = `select distinct(district) as district, inspection.date_of_inspection as inspection_date, cast(avg( inspection.school_management_committee) as unsigned) +
