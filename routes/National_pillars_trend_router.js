...
router.get('/:region', function(req, res, next) {
    ...
    const P2Query = `select distinct(region) as region, inspection.date_of_inspection as inspection_date, cast(avg(inspection.teacher_and_pupil_attendance) as unsigned) +
    ...
    const P3Query = `select distinct(region) as region, inspection.date_of_inspection as inspection_date, cast(avg(inspection.lesson_planning) as unsigned) +
    ...
    const P4Query = `select distinct(region) as region, inspection.date_of_inspection as inspection_date, cast(avg( inspection.school_management_committee) as unsigned) +
    ...
    connection.query(P1Query, function fillGraph(err, result, ) {
        ...
        res.send({region: region, pillar1Score: pillar1Score, inspections: inspections})
    })
